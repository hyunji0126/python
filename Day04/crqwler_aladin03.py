...
file_path = f'C:/test/crawling/알라딘 베스트셀러 1~400위(뉴버젼)_{d.year}_{d.month}_{d.day}.xslx'
...
# 엑셀 처리 선언
# Workbook 객체를생성하여 엑셀 파일을 하나 생성( 매개값으로 저장될 경로를 지정.)
...
# 브라우저 설정 - 일반 모드 
broswer = webdriver.Chrome('C:/test/crawling/chromedriver.exe')

# 브라우저 설정  - headless 모드
# broswer = webdriver.Chrome('C:/test/crawling/chromedriver.exe', options=chrome_options)
...
# 페이지 이동 (베스트셀러 페이지)
broswer.get('https://www.aladin.co.kr/shop/common/wbest.aspx?BranchType=1')
